...
description: A 股 B1 买点日更全流程——活跃市值闸门 → 更新最新行情 → 全市场 B1 扫描 → 分级评价（闸门决定这批命中怎么读）。触发场景:(1) 用户说「跑一下 B1」「B1 选股」「今天有哪些 B1 买点」「扫一下 B1」(2) 用户要行情更新 + B1 选股一次性完成(3) 用户问「今天的 B1 结果怎么样」「B1 复盘」(4) 用户输入 /qk-stock-b1-daily。
---
