...
def macd(
    ...
    macd_difference = macd_diff(close=close, window_slow=window_slow, window_fast=window_fast, window_sign=window_sign, fillna=fillna)*2
    
    df = pd.DataFrame({
        'macd_dif': macd_line,
        'macd_dea': signal_line,
        'macd_difference': macd_difference
    }).round(2)
...
def add_macd_to_dataframe(
    ...
    Args:
        ...
        window_fast: int, the period for the fast EMA, default is 12
        window_sign: int, the period for the signal line, default is 9
        macd_cols: list of str, names for the new MACD columns, default is ['macd_dif', 'macd_dea', 'macd_difference']
        fillna: bool, whether to fill NaN values, default is False
    ...
    if inplace:
        for col_name, macd_col in zip(['macd_dif', 'macd_dea', 'macd_difference'], macd_df.columns):
