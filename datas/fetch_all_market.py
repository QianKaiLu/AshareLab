...
REQUEST_DELAY = 0.5  # seconds between tushare requests to avoid burst rate limits

def fetch_stock_bars_parallel(stock_codes, source="tushare"):
...
def worker_fetch_stock_and_queue(code, result_queue, source="tushare"):
    ...
    if latest_date is not None:
        ...
        if latest_date.date() >= to_date:
            return True
        from_date = (latest_date - pd.Timedelta(days=5)).strftime("%Y%m%d")

    # Spread out burst requests
    time.sleep(REQUEST_DELAY)

    try:
        df = None
        if source == "akshare":
            df = fetch_daily_bar_from_akshare(code=code, from_date=from_date)
        elif source == "tushare":
            last_adj_close = None
            if latest_date is not None:
                last_bars = query_latest_bars(code, n=1)
                if not last_bars.empty:
                    last_adj_close = float(last_bars['close'].iloc[-1])
            df = fetch_daily_bar_from_tushare(
                code=code, from_date=from_date, last_adjusted_close=last_adj_close)
...
def database_writer(result_queue, stop_event):
