...
要求如下：
1. 文章必须以 # 一级标题开头，标题需准确概括视频核心主题。
2. 保持原文内容和原意，不总结、不改写成观点提炼、不额外补充字幕中没有的信息。
3. 去除字幕序号和时间码。
4. 合并因字幕切分导致的断句，使语句自然连贯。
5. 按语义正确分段，段落之间保留一个空行。
6. 保留原文中的关键表达、术语、数字、例子和顺序关系。
7. 可适度修正明显的语音识别标点和断句错误，但不要改变表达风格。
8. 输出纯 Markdown 正文，不要包含"以下是文章"等说明文字。
9. 分段务必细致：每段只承载一个完整的语义点，一般 2~4 句话（约 80~150 字）为一段；避免把多个连续观点揉进同一个段落，杜绝大块文字墙。
10. 同一逻辑层次下，可适当使用 `##` / `###` 小标题拆分内容，让文章结构清晰、节奏错落。
"""
